...
static duk_ret_t duk__console_log_helper(duk_context *ctx, const char *error_name, int level)
{
	...
	appsrv_log(level, "%s", duk_to_string(ctx, -1));
	return 0;
}
...
static duk_ret_t duk__console_assert(duk_context *ctx)
{
	...
	return duk__console_log_helper(ctx, "AssertionError", APPSRV_LOG_DEBUG);
}

static duk_ret_t duk__console_log(duk_context *ctx)
{
	return duk__console_log_helper(ctx, NULL, APPSRV_LOG_DEBUG);
}

static duk_ret_t duk__console_trace(duk_context *ctx)
{
	return duk__console_log_helper(ctx, "Trace", APPSRV_LOG_DEBUG);
}

static duk_ret_t duk__console_info(duk_context *ctx)
{
	return duk__console_log_helper(ctx, NULL, APPSRV_LOG_INFO);
}

static duk_ret_t duk__console_warn(duk_context *ctx)
{
	return duk__console_log_helper(ctx, NULL, APPSRV_LOG_WARN);
}

static duk_ret_t duk__console_error(duk_context *ctx)
{
	return duk__console_log_helper(ctx, "Error", APPSRV_LOG_ERROR);
}

static duk_ret_t duk__console_dir(duk_context *ctx)
{
	/* For now, just share the formatting of .log() */
	return duk__console_log_helper(ctx, 0, APPSRV_LOG_DEBUG);
}
